...
model.save('{}/whole_data_model.h5'.format(out_dir))
...
graph_data.to_csv('{}/whole_data_acc_loss.csv'.format(out_dir), index=False, header=True)
...
plt.grid()
plt.legend(['train', 'val'], loc='upper left')
plt.savefig('{}/whole_data_acc_los.pdf'.format(out_dir))
# plot AUC
...
plt.grid()
plt.savefig('{}/whole_data_ROC.pdf'.format(out_dir))
roc_data = pd.DataFrame(columns=['fpr', 'tpr'])
roc_data['fpr'] = fpr
roc_data['tpr'] = tpr
roc_data.to_csv('{}/whole_data_roc.csv'.format(out_dir), index=False, header=True)
